...
def main():
    ...
    #
    # ONLY the pages being added are checked. This used to gate the whole of
    # auto.json, which meant that on 11 September 2026 two new gate rules failed 55
    # already-published pages written before those rules existed, and the publish
    # aborted on every run - nothing reached the site for hours, including pages
    # with nothing wrong with them. Existing pages get re-checked when they are
    # next rewritten, which is the right moment to hold them to a new standard.
    gate = subprocess.run(
